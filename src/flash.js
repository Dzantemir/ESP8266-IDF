...
async function runFlash(action = 'flash', eraseFirst = false) {
    ...
    // NOTE: --before / --after / --flash_mode / --flash_freq / --flash_size / -z / -u
    // are all esptool.py options, NOT idf.py options. idf.py flash reads flash settings
    // from build/flasher_args.json (generated by menuconfig). All flash parameters
    // (baud, mode, freq, size, compression, reset behaviour) must be configured via
    // menuconfig → Serial flasher config. The extension no longer exposes manual
    // overrides for these — they caused "No such option: --flash_mode" errors when
    // passed to idf.py.
    ...
    // #FIX(1.85.3): erase_flash DOES need build/flasher_args.json — idf.py's
    // erase_flash action calls _get_esptool_args() which opens it. This is
    // proven by the FileNotFoundError traceback users see on a clean build dir:
    //   File "idf.py", line 335, in _get_esptool_args
    //     with open(os.path.join(args.build_dir, "flasher_args.json")) as f:
    //   FileNotFoundError: ...build\flasher_args.json
    //
    // The 1.85.2 fix was WRONG: it assumed erase_flash needed no build artifacts
    // and only checked for the build/ DIRECTORY. When build/ existed but
    // flasher_args.json was missing inside it (e.g. after fullclean removed
    // generated files, or a partial/corrupt build state), erase_flash ran
    // directly and crashed with the traceback above.
    //
    // Corrected logic: check the actual file erase_flash needs — flasher_args.json.
    // When it's missing, a full build is overkill — `reconfigure` regenerates
    // flasher_args.json via the CMake configure step without compiling. Chain
    // reconfigure → erase_flash. This also fixes the second bug: the generic
    // flasher_args.json check below ran a full build for erase_flash but then
    // DROPPED the erase_flash command (postBuildAction='none' for erase_flash),
    // so the erase never actually ran.
    if (baseAction === 'erase_flash') {
        const flasherArgsJsonEf = path.join(rootFlash, 'build', 'flasher_args.json');
        if (!fs.existsSync(flasherArgsJsonEf)) {
            vscode.window.showInformationMessage(
                'ESP: build/flasher_args.json not found — running reconfigure before erase_flash.'
            );
            // reconfigure regenerates flasher_args.json (CMake configure step);
            // chain erase_flash after it via chainArgs so the erase actually runs.
            // #FIX(1.85.0): _alreadyBusy=true — runFlash already holds the busy lock.
            await runIdf(['reconfigure'], 'ESP › Reconfigure → Erase flash',
                false, {}, args, null, null, true);
        } else {
            // flasher_args.json exists — erase_flash can run directly.
            // #FIX(1.85.0): _alreadyBusy=true — runFlash already holds the busy lock.
