...
from fatfs_utils.utils import MAX_EXT_SIZE
from fatfs_utils.utils import MAX_NAME_SIZE
...
from fatfs_utils.exceptions import NotInitialized
from wl_fatfsgen import remove_wl


def build_file_name(name1, name2, name3):
    # type: (bytes, bytes, bytes) -> str
    full_name_ = name1 + name2 + name3
    # need to strip empty bytes and null-terminating char ('\x00')
    stripped = full_name_.rstrip(FULL_BYTE)
    # Ensure even number of bytes for UTF-16 decoding
    if len(stripped) % 2 != 0:
        stripped = stripped[:-1]
    return stripped.decode('utf-16-le', errors='replace').rstrip('\x00')


def get_obj_name(obj_, directory_bytes_, entry_position_, lfn_checksum_, long_name_support=False):
    # type: (dict, bytes, int, int, bool) -> str
    obj_ext_ = obj_['DIR_Name_ext'].rstrip(chr(PAD_CHAR))
    ext_ = '.%s' % obj_ext_ if len(obj_ext_) > 0 else ''
    obj_name_ = obj_['DIR_Name'].rstrip(chr(PAD_CHAR)) + ext_  # short entry name
    ...
    for pos in range(entry_position_ - 1, -1, -1):  # loop from the current entry back to the start
        obj_address_ = FATDefaults.ENTRY_SIZE * pos
        entry_bytes_ = directory_bytes_[obj_address_: obj_address_ + FATDefaults.ENTRY_SIZE]
...
def traverse_folder_tree(
    directory_bytes_, name, state_, fat_, binary_array_, long_name_support=False
):
    # type: (bytes, str, BootSectorState, FAT, bytes, bool) -> None
    os.makedirs(name, exist_ok=True)

    assert len(directory_bytes_) % FATDefaults.ENTRY_SIZE == 0
    entries_count_ = len(directory_bytes_) // FATDefaults.ENTRY_SIZE

    for i in range(entries_count_):
        obj_address_ = FATDefaults.ENTRY_SIZE * i
        try:
            obj_ = Entry._parse_entry(
                directory_bytes_[obj_address_: obj_address_ + FATDefaults.ENTRY_SIZE]
            )
        except (struct.error, UnicodeDecodeError):
            long_name_support = True
            continue

        if obj_['DIR_Attr'] == 0:  # empty entry
            continue

        obj_name_ = get_obj_name(
            obj_,
            directory_bytes_,
            entry_position_=i,
            lfn_checksum_=lfn_checksum((obj_['DIR_Name'] + obj_['DIR_Name_ext']).ljust(MAX_NAME_SIZE + MAX_EXT_SIZE, ' ')),
...
def remove_wear_levelling_if_exists(fs_):
    # type: (bytes) -> bytes
    """
    ...
    try:
        ...
        if boot_sector__.boot_sector_state.size == len(fs_):
            return fs_
    except (UnicodeDecodeError, NotInitialized):
        pass
    plain_fs = remove_wl(fs_)
    return plain_fs
...
if __name__ == '__main__':
    ...
    argument_parser.add_argument('--long-name-support', action='store_true', default=True, help='Enable long name support (VFAT LFN). Enabled by default.')
    argument_parser.add_argument('--no-long-name-support', action='store_false', dest='long_name_support', help='Disable long name support.')
    ...
    traverse_folder_tree(
        ...
        fat,
        fs,
        long_name_support=args.long_name_support,
    )
